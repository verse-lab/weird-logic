...
def r2 : ContextFreeRule trm String :=
  {
    input := "S1",
    output := [Symbol.terminal ExampleFor.trm1, Symbol.nonterminal "S1", Symbol.terminal ExampleFor.trm2],
  }
...
def cfg1 : ContextFreeGrammar trm :=
  {
    NT := String,
    initial := "S1",
    rules := Finset.mk {r1, r2, r3, r4} (by unfold r1 r2 r3 r4 ExampleFor.trm1 ExampleFor.trm2; simp)
  }
...
lang_def' prog_c1 :=
  fun ⸨xl: Loc⸩ ⸨yl: Loc⸩ ⸨pa:Val⸩ ⸨pb:Val⸩=>
    ExampleFor.prog_c1(⸨xl:Loc⸩, ⸨yl:Loc⸩, pa);
    ExampleIf.prog_c1(⸨xl:Loc⸩, ⸨yl:Loc⸩, pb)

def orexp_grammar : Bool → trm
  | true => ExampleIf.trm1
  | false => ExampleIf.trm2
...
def lang_sn1 i :=
  ExampleFor.cfgexp_grammar i ExampleFor.trm1 ExampleFor.trm2
...
lemma vulnwm_spec (xv : ℤ) (yv : ℤ):
  ...
  { simp
    ...
    { clear tmp
      ...
      unfold ExampleFor.prog_c1
      xwp; xseq_xlet_if_needed
      apply xfor_empty_lemma ; omega
      xsimp
      apply xfor_empty_lemma ; omega
      xsimp
      unfold ExampleIf.prog_c1
  ...
  { simp ; intro i j hneq ; simp [Set.singleton, trm_funs, lang_sn1]
    intro heq ; apply ExampleFor.cfgexp_grammar_injective at heq
    · contradiction
    · unfold ExampleFor.trm1 ; simp
    · unfold ExampleFor.trm2 ; simp
  }
  ...
  apply weird_gram_seq_full_prod
    ...
    (β := (trm × payload)ˡ)
    (γ := (trm × payload)ˡ)
    (sht_prog1 := LGTM.SHT.mk ⟪0, ExampleFor.pay_index⟫
      fun p ↦ ExampleFor.prog_c1.trm_call [xl, yl, [lang| ⟨p.val.2⟩]])
    (sht_lang1 := LGTM.SHT.mk ⟪1, ExampleFor.lang_index⟫
      fun l ↦ l.val.1.trm_call [xl, yl])
    (sht_prog2 := LGTM.SHT.mk ⟪0, ExampleIf.pay_index⟫
      fun p ↦ ExampleIf.prog_c1.trm_call [xl, yl, [lang| ⟨p.val.2⟩]])
    (sht_lang2 := LGTM.SHT.mk ⟪1, ExampleIf.lang_index⟫
      fun l ↦ l.val.1.trm_call [xl, yl])
    ...
    (Q1 := fun n =>
      ...
      hforall fun (_ :
      n ∈ (({⟨0, (default_trm, j)⟩} ∪
      {⟨1, ([lang| fun ⸨xl: Loc⸩ ⸨yl : Loc⸩=> {ExampleFor.cfgexp_grammar j ExampleFor.trm1 ExampleFor.trm2}], default_payload)⟩}) : Set (trm × payload)ˡ)) =>
  ...
  { clear aux haux
    ...
    simp [lang_index, lang_squeeze_list',
      ExampleFor.lang_index, ExampleFor.lang_fun_list, ExampleFor.lang_squeeze_list,
      ExampleIf.lang_index, ExampleIf.lang_index'_unfold,
  ...
  { clear aux haux
    ext a ; dsimp only ; rcases a with ⟨⟨al, ⟨atrm, ap⟩⟩, ⟨bl, ⟨btrm, bp⟩⟩⟩
    simp [pay_index,
      ExampleFor.pay_index,
      ExampleIf.pay_index, ExampleIf.pay_index',
      default_trm]
    simp [exists_labType]
    aesop
  }
  ...
  { simp ; ext a ; rcases a with ⟨al, ⟨atrm, ap⟩⟩
    simp [ExampleFor.pay_index, ExampleFor.lang_index, ExampleFor.lang_fun_list, ExampleFor.lang_squeeze_list,
  ...
  { dsimp only
    ...
    simp [ExampleFor.lang_index, ExampleFor.lang_fun_list, ExampleFor.lang_squeeze_list] at lin
    ...
    on_goal 2=> simp [ExampleFor.pay_index]
    specialize hpre n (by simp)
    constructor
    on_goal 1=> simp [ExampleFor.pay_index]
    -- trick
  ...
  { dsimp
    ...
    simp [ExampleFor.pay_index, ExampleFor.lang_index, ExampleFor.lang_fun_list, ExampleFor.lang_squeeze_list] at hin
    constructor
  ...
  { dsimp only
    ...
    { congr! 3 ; rw [ExampleFor.cfgexp_grammar_isubst] ; rfl }
    { cases p2 <;> rfl }
  }
  { simp }
  { apply ExampleFor.forwm_spec }
  { rintro ⟨jl, ⟨jtrm, jp⟩⟩ jin
    simp [ExampleFor.pay_index, ExampleFor.lang_index, ExampleFor.lang_fun_list, ExampleFor.lang_squeeze_list] at jin
    rcases jin with ⟨_, ⟨n, jin⟩, _⟩ ; subst jl jp ; symm at jin
    dsimp only
    -- involving the oracle here
    apply LGTM.triple_conseq
    on_goal 3=> apply ExampleIf.ifwm_spec (xv + Int.ofNat (aux jtrm |>.getD 0)) (yv + Int.ofNat (aux jtrm |>.getD 0))
    ...
    { intro hv hh hpre ; simp
      ...
      simp [ExampleIf.lang_index, ExampleIf.lang_index'_unfold, ExampleIf.pay_index, exists_labType] at hpre ⊢
