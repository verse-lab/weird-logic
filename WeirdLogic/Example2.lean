...
def pay_idx_prod : Set (trm × multi_payload × multi_payload) :=
  {
...
def lang_idx_prod : Set (trm × multi_payload × multi_payload ):=
  { (l, ([0],[0])) | l ∈ lang_set}

variable (pa_len : ℕ) (pb_len : ℕ) (xptr : loc)
variable (paptr : trm × multi_payload × multi_payload -> loc) (pbptr : trm × multi_payload × multi_payload -> loc)

lemma example4_spec' (f : ℤ -> val) (g q : ℤ -> val):
  {
    arr⟨⋆⟩(xptr , i in 1 =>f i) ∗
    bighstar ⟪0,pay_idx_prod⟫ (fun i => harray i.val.2.1 (paptr i.val) ∗ harray i.val.2.2 (pbptr i.val))
  }
  [0| p in pay_idx_prod => prog_c1(⟨f 0⟩,⸨pa:Loc⸩, ⸨pb:Loc⸩)]
